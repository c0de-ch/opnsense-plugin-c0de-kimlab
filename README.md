...
- **Direct domain** (e.g. `hostname.direct-domain-example.com`) — real device IP for SSH, ping, direct access
- **Proxy domain(s)** (e.g. `hostname.proxy-domain-example.com`) — reverse proxy IP for HTTPS via Caddy
...
2. Enable the plugin and configure:
   - **Direct Domain** — domain for real device IPs (e.g. `direct-domain-example.com`)
   - **Proxy Domains** — space-separated reverse proxy domains (e.g. `proxy-domain-example.com`)
